...
if ($PSVersionTable.PSEdition -eq 'Core') {
    Write-Host "Win11Debloat yêu cầu Windows PowerShell 5.1, nhưng hệ thống đang chạy PowerShell $($PSVersionTable.PSVersion) (phiên bản pwsh / Core)." -ForegroundColor Red
    Write-Host "Việc gỡ cài đặt ứng dụng và tính năng điểm khôi phục hệ thống (restore point) phụ thuộc vào các module không có sẵn trong PowerShell 7, vì vậy script không thể hoàn thành chính xác ở đây." -ForegroundColor Red
    Write-Host "Vui lòng chạy lại script này bằng Windows PowerShell thông thường (powershell.exe)." -ForegroundColor Yellow
    exit 1
}
...
if (-not $isAdmin) {
    Write-Host "Win11Debloat phải được chạy dưới quyền Quản trị viên (Administrator)." -ForegroundColor Red

    $choice = Read-Host "Bạn có muốn khởi động lại script dưới quyền Quản trị viên không? (y/n)"
    ...
    if ($choice -match '^[Yy]$') {
        ...
        try {
            ...
            Write-Error "Không thể khởi động Win11Debloat dưới quyền Quản trị viên: $_"
            Exit 1
        }
...
if ($ExecutionContext.SessionState.LanguageMode -ne "FullLanguage") {
    Write-Error "Win11Debloat không thể chạy trên hệ thống của bạn, việc thực thi PowerShell đang bị hạn chế bởi các chính sách bảo mật"
    Write-Output "Nhấn phím bất kỳ để thoát..."
...
if ($env:PATH -notmatch "(?i)(^|;)$([regex]::Escape($system32Path))(?=;|$)") {
    ...
    Write-Warning "Đường dẫn System32 bị thiếu trong biến môi trường PATH, nó đã được tự động thêm vào cho phiên chạy này."
}
...
Write-Host ""
Write-Host "             Win11Debloat đang khởi chạy..." -ForegroundColor White
Write-Host "                Vui lòng không tắt cửa sổ chương trình đang chạy." -ForegroundColor DarkGray
Write-Host ""
Write-Host ""
...
if (-not $WhatIfPreference) {
    ...
    if ($gpoExecutionPolicySet) {
        ...
        if ($markedScriptFiles.Count -gt 0) {
            Write-Host "Đang bỏ chặn (unblock) $($markedScriptFiles.Count) tệp PowerShell..."
            $unblockErrors = @()
            $markedScriptFiles | Unblock-File -ErrorAction SilentlyContinue -ErrorVariable +unblockErrors

            if ($unblockErrors.Count -gt 0) {
                Write-Warning "Không thể bỏ chặn $($unblockErrors.Count) tệp PowerShell."
            }
            else {
                Write-Host "Tất cả các tệp đã được bỏ chặn thành công."
            }
        }
    }
}
...
try {
    ...
    if ($null -ne $computerSystem -and $computerSystem.PartOfDomain) {
        Write-Warning "Máy tính này đã tham gia domain (Domain-joined). Group Policy có thể sẽ ghi đè lên các thiết lập của Win11Debloat."
    }
}
catch { }
...
if (-not ((Test-Path $script:DefaultSettingsFilePath) -and (Test-Path $script:AppsListFilePath) -and (Test-Path $script:RegfilesPath) -and (Test-Path $script:AssetsPath) -and (Test-Path $script:AppSelectionSchema) -and (Test-Path $script:ApplyChangesWindowSchema) -and (Test-Path $script:SharedStylesSchema) -and (Test-Path $script:BubbleHintSchema) -and (Test-Path $script:RestoreBackupWindowSchema) -and (Test-Path $script:FeaturesFilePath) -and (Test-Path $script:DefaultLanguagePath))) {
    Write-Error "Win11Debloat không tìm thấy các tệp tin cần thiết, vui lòng đảm bảo tất cả các tệp script đều đầy đủ."
    Write-Output "Nhấn phím bất kỳ để thoát..."
    $null = [System.Console]::ReadKey()
    Exit 1
}
...
try {
    ...
    foreach ($feature in $featuresData.Features) {
        if ([string]::IsNullOrWhiteSpace([string]$feature.FeatureId) -or [string]::IsNullOrWhiteSpace([string]$feature.Label) -or [string]::IsNullOrWhiteSpace([string]$feature.ApplyText)) {
            Write-Warning "Tính năng '$($feature.FeatureId)' bị thiếu FeatureId, Label, hoặc ApplyText trong tệp Features.json và sẽ bị bỏ qua."
            continue
        }
    ...
    Write-Error "Không thể tải cấu hình tính năng từ tệp Features.json"
    Write-Output "Nhấn phím bất kỳ để thoát..."
    $null = [System.Console]::ReadKey()
    Exit 1
}
...
try {
    ...
    Write-Error "Không thể xác định xem WinGet đã được cài đặt hay chưa, lệnh winget thất bại: $_"
    $script:WingetInstalled = $false
}
...
if (-not $script:WingetInstalled -and -not $Silent) {
    Write-Warning "WinGet chưa được cài đặt hoặc đã lỗi thời, điều này có thể khiến Win11Debloat không thể gỡ cài đặt một số ứng dụng."
    Write-Output ""
    Write-Output "Nhấn phím bất kỳ để tiếp tục bằng mọi giá..."
    $null = [System.Console]::ReadKey()
}
...
# Load the GUI's language content. -Language overrides the default language (vi-VN),
# falling back to en-US either way if the requested language isn't available.
$script:Lang = if ($Language) {
    Import-LanguageFile -LanguageCode $Language
} elseif (Test-Path (Join-Path $script:LanguagesPath 'vi-VN')) {
    Import-LanguageFile -LanguageCode 'vi-VN'
} else {
    Import-LanguageFile
}
...
if (-not ($script:Params.ContainsKey("Verbose"))) {
    ...
    Write-Host "Chế độ chi tiết (Verbose) đã được bật"
    Write-Output ""
    Write-Output "Nhấn phím bất kỳ để tiếp tục..."
    $null = [System.Console]::ReadKey()
...
if ($script:Params.ContainsKey("Sysprep")) {
    ...
    if ($WinVersion -lt 22000) {
        Write-Error "Chế độ Sysprep của Win11Debloat không được hỗ trợ trên Windows 10"
...
if ((-not $script:Params.Count) -or $RunDefaults -or $RunDefaultsLite -or $RunSavedSettings -or $Config -or ($controlParamsCount -eq $script:Params.Count)) {
    ...
    elseif ($RunSavedSettings) {
        if (-not (Test-Path $script:SavedSettingsFilePath)) {
            ...
            Write-Error "Không tìm thấy tệp LastUsedSettings.json, không có thay đổi nào được thực hiện"
            Wait-ForKeyPress -ExitCode 1
        }
    ...
    elseif ($Config) {
        ...
        if ($launchInCLI) {
            ...
            try {
                ...
                Write-Warning "Không thể khởi động giao diện đồ họa: $($_.Exception.Message)"
                Write-Verbose "Chi tiết lỗi GUI: $($_.Exception.ToString())"
                if (-not $Silent) {
                    Write-Host ""
                    Write-Host "Nhấn phím bất kỳ để tiếp tục trong chế độ dòng lệnh (CLI)..."
                    $null = [System.Console]::ReadKey()
                }
...
if (($controlParamsCount -eq $script:Params.Keys.Count) -or ($script:Params.Keys.Count -eq 1 -and ($script:Params.Keys -contains 'CreateRestorePoint' -or $script:Params.Keys -contains 'Apps'))) {
    Write-Output "Script đã hoàn tất mà không thực hiện bất kỳ thay đổi nào."
    Wait-ForKeyPress
}
...
if ($script:CancelRequested) {
    Write-Warning "Quá trình thực thi script đã bị người dùng hủy. Các thay đổi còn lại chưa được áp dụng."
    Wait-ForKeyPress
}
...
Write-Output ""
Write-Output ""
Write-Output "Đã hoàn tất script! Vui lòng kiểm tra các thông báo phía trên nếu có lỗi."
